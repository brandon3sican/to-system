                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('positions.update', $position) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input id="name" type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $position->name) }}" required>
                        ...
                        <div class="mb-3">
                            ...
                            <select id="div_sec_unit_id" class="form-control @error('div_sec_unit_id') is-invalid @enderror" name="div_sec_unit_id" required>
                                ...
                                @foreach($divSecUnits as $divSecUnit)
                                    <option value="{{ $divSecUnit->id }}" {{ old('div_sec_unit_id', $position->div_sec_unit_id) == $divSecUnit->id ? 'selected' : '' }}>
                                        {{ $divSecUnit->name }}
                                    </option>
                                @endforeach
                            </select>
